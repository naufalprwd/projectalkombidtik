...
<?php
	...
	switch($act){
		...
		case "delete_data_pinjaman":
            ...
            $config->query('insert into history (nip,kode_jenis,nama,pangkat,jabatan,tipe_barang,jenis_barang
		...
		case "add_data_pinjaman":
		  if($_SERVER["REQUEST_METHOD"]=="POST"){
			$result = $config->query('insert into pinjaman(nip,kode_jenis,nama,pangkat,jabatan,tipe_barang,jenis_barang,nup,tanggal_pinjam,tanggal_kembali,jumlah,peruntukan) 
									values("'.$_POST['nip'].'", "'.$_POST['kode_jenis'].'", "'.$_POST['nama'].'","'.$_POST['pangkat'].'","'.$_POST['jabatan'].'","'.$_POST['tipe_barang'].'","'.$_POST['jenis_barang'].'","'.$_POST['nup'].'","'.$_POST['tanggal_pinjam'].'","'.$_POST['tanggal_kembali'].'","'.$_POST['jumlah'].'","'.$_POST['peruntukan'].'")');
		...
		case "add_petugas";
		 if($_SERVER["REQUEST_METHOD"]=="POST"){
			$result = $config->query('insert into datapegawai(nip,nama,jabatan,email,telpon,pangkat,password) 
                            		values( "'.$_POST['nip'].'", "'.$_POST['nama'].'", "'.$_POST['jabatan'].'", "'.$_POST['email'].'", "'.$_POST['telpon'].'", "'.$_POST['pangkat'].'", "'.$_POST['password'].'")');
			if($result>0){
		...
		case "update_data_petugas":
			$result = $config->update("datapegawai",'nip="'.$_POST['nip'].'", nama="'.$_POST['nama'].'",jabatan="'.$_POST['jabatan'].'",email="'.$_POST['email'].'",telpon="'.$_POST['telpon'].'",pangkat="'.$_POST['pangkat'].'",password="'.$_POST['password'].'"',"id_data",$_POST['id']);
			if($result>0){
		...
		case "update_data_admin":
			$result = $config->update("admin",'nip="'.$_POST['nip'].'", nama="'.$_POST['nama'].'",jabatan="'.$_POST['jabatan'].'",email="'.$_POST['email'].'",telpon="'.$_POST['telpon'].'",pangkat="'.$_POST['pangkat'].'",username="'.$_POST['username'].'",password="'.$_POST['password'].'"',"id_admin",$_POST['id']);
			if($result>0){
